...
{
  if (argint(0, &scheduler_type) < 0)
  {
    cprintf("sys_set_sched() failed!\n");
  }
  set_prio(scheduler_type);
  return 0;
}
...
void sys_pause_scheduling(void) {
  int pause;
  argint(0, &pause);
  pause_scheduling(pause);
}

int sys_mlfq_set_allotment(void){
  int priority, allotment;
  argint(0, &priority);
  argint(1, &allotment);
  return mlfq_set_allotment(priority, allotment);
}
